...
4. サービスの起動
   生成されたバイナリを実行します。デフォルトではWebサーバーが8080番ポート、SFTPサーバーが2022番ポートで待機します。
...
- `listen`: Web UIおよびAPIサーバーが待機するアドレスとポートです。
- `sftp.listen`: ファイル操作用SFTPサーバーが待機するアドレスとポートです。
...
- `discord`: Bot用トークンや、特定の操作を行うチャンネルIDを設定します。
